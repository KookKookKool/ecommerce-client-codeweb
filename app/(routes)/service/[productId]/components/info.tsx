...
const Info = ({ product }: InfoProp) => {
  ...
  return (
    <div>
        ...
        <div className="col-span-3 space-y-8">
          ...
          {[1].map((num) => (
            <div
              key={num}
              className={cn(
                "w-8 h-8 cursor-pointer rounded-md flex items-center justify-center border border-primary", qty === num 
      ...
      className="w-full py-6text-xl font-semibold hover:bg-primary hover:text-white flex items-center justify-center gap-3">Add to cart<ShoppingCart className="w-4 h-4" /></Button>
    </div>
  );
};
